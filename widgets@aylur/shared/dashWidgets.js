...
class DashWidget extends St.BoxLayout {
    ...
    _sync() {
        ...
        const width  = this._settings.get_int(`dash-${this._module}-width`);
        const height = this._settings.get_int(`dash-${this._module}-height`);
        this.style = `
    ...
    _parseAlign(axis) {
        const num = this._settings.get_int(`dash-${this._module}-${axis}-align`);
        switch (num) {
...
class LinksWidget extends DashWidget {
    ...
    _sync() {
        ...
        const names = this._settings.get_strv('dash-links-names');
        const urls  = this._settings.get_strv('dash-links-urls');
...
class AppsWidget extends DashWidget {
    ...
    _buildUI() {
        const rows = this._settings.get_int('dash-apps-rows');
        const cols = this._settings.get_int('dash-apps-cols');
...
class SettingsWidget extends DashWidget {
    ...
    _sync() {
        ...
        const iconSize = this._settings.get_int('dash-settings-icon-size');
...
class SystemWidget extends DashWidget {
    ...
    _sync() {
        ...
        const iconSize = this._settings.get_int('dash-system-icon-size');
        const layout = this._settings.get_int('dash-system-layout');
