...
class WorkspacesIndicator extends PanelMenu.Button {
    ...
    _sync() {
        ...
        for (let index = 0; index < ws_count; ++index) {
            const active = index === active_ws_index;

            const ws_btn = new St.Button({
                can_focus: true,
                style_class: active
...
var MyExtension = class MyExtension {
    ...
    _reload() {
        ...
        const pos    = this._settings.get_int('workspace-indicator-position');
        const offset = this._settings.get_int('workspace-indicator-offset');
