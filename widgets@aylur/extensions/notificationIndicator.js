...
class Indicator extends St.BoxLayout {
    ...
    _syncCounter() {
        const count = this.list._messages.length;
...
class PanelButton extends PanelMenu.Button {
    _init(settings) {
        ...
        const maxHeight = Main.layoutManager.primaryMonitor.height - Main.panel.height;
...
var MyExtension = class MyExtension extends Extension {
    ...
    _reload() {
        ...
        const pos = this._settings.get_int('notification-indicator-position');
        const offset = this._settings.get_int('notification-indicator-offset');
