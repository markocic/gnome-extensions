...
class MediaButton extends PanelMenu.Button {
    _init(settings) {
        ...
        if (settings.get_boolean('media-player-show-player-icon')) {
            const iconPos = settings.get_int('media-player-player-icon-position');
            iconPos > 0
        ...
        const maxWidth = settings.get_int('media-player-max-width');
        if (maxWidth > 0)
    ...
    _sync() {
        const mpris = this.media.getPreferred();
        if (mpris) {
            this.player = mpris;
...
class MediaControls extends PanelMenu.Button {
    _init(settings) {
        ...
        const preferred = settings.get_string('media-player-prefer');
